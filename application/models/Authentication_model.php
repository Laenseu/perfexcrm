        ...
        if ((!empty($email)) and (!empty($password))) {
            ...
            if ($user) {
                ...
                if (!app_hasher()->CheckPassword($password, $user->password)) {
                    ...
                    // Set loginnumber to 2 if the user logs in between 10:30 AM and 5:30 PM
                    if ($currentTime >= strtotime('7:00:00') && $currentTime <= strtotime('10:00:00')) {
                        $loginNumber = 1;
                    } else {
                        $loginNumber = 2; // Default to 1 for other times
                    }
            
                    // Update loginnumber in the user table
                    $this->db->set('loginnumber', $loginNumber);
                    $this->db->where('email', $email);
                    $this->db->update($table);
            
                    // Update last login date for staff
                    $this->db->set('last_login', date('Y-m-d'));
                    $this->db->where('staffid', $user->staffid);
                    $this->db->update('staff');
            
                    // Check if the staff member has already logged in today
                    $this->db->where('staffid', $user->staffid);
                    $this->db->where('DATE(timeanddate)', date('Y-m-d'));
                    $existingLogin = $this->db->get('tblstaff_login_info')->row();
            
                    if (!$existingLogin) {
                        // Insert login information into staff_login_info table
                        $this->db->insert('tblstaff_login_info', [
                            'staffid'      => $user->staffid,
                            'loginnumber'  => $loginNumber,
                            'timeanddate'  => date('Y-m-d H:i:s'),
                            'hours'        => ($loginNumber == 1) ? 8 : 4, // Assuming 8 hours for loginnumber 1, and 4 hours for loginnumber 2
                        ]);
                    }
                }
            }
